...
@section('content')
<div class="max-w-6xl mx-auto mt-10" x-data="{ modalOpen: false, modalImage: '' }">
    ...
    <table class="w-full border-collapse bg-white text-sm text-left">
      <thead class="bg-gray-100 text-gray-700 uppercase text-xs">
        <tr>
          ...
          <th class="px-4 py-1">DNI</th>
          <th class="px-4 py-1">Email</th>
          <th class="px-4 py-1">Teléfono</th>
          <th class="px-4 py-1">Universidad</th>
          <th class="px-4 py-1">Carrera</th>
          <th class="px-4 py-1">Comisión</th>
          <th class="px-4 py-1">GitHub</th>
          <th class="px-4 py-1">LinkedIn</th>
          <th class="px-4 py-1 text-center">WhatsApp</th>
          <th class="px-4 py-1">Materias</th>
          <th class="px-4 py-1 text-center">Acciones</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-50">
          ...
          <td class="px-4 py-3 font-medium text-gray-900">{{ $alumno->name }}</td>
          <td class="px-4 py-3">{{ $alumno->dni }}</td>
          <td class="px-4 py-3">{{ $alumno->email }}</td>
          <td class="px-4 py-3">{{ $alumno->telefono }}</td>
          <td class="px-4 py-3">{{ $alumno->university }}</td>
          <td class="px-4 py-3">{{ $alumno->career }}</td>
          <td class="px-4 py-3">{{ $alumno->commission }}</td>
          <td class="px-4 py-3">
            @if($alumno->github)
            <a href="{{ $alumno->github }}" target="_blank" class="text-blue-600 underline">
              {{ $alumno->github }}
            </a>
            @else
          ...
          <td class="px-4 py-3">
            @if($alumno->linkedin)
            <a href="{{ $alumno->linkedin }}" target="_blank" class="text-blue-600 underline">
              {{ $alumno->linkedin }}
            </a>
            @else
            <span class="text-gray-400">No registrado</span>
  ...
  <div x-show="modalOpen"
    ...
    x-transition>
    <div class="relative">
      <img :src="modalImage" alt="Ampliada" class="max-w-lg max-h-[80vh] rounded-lg shadow-lg">
